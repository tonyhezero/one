...
//	当连接开始时计时，通告社团检测对象：一个新的连接开始了
	...
	public void connectionDown(DTNHost thisHost, DTNHost peer)
	{
		...
		if(!connHistory.containsKey(peer))
		{
			...
			System.out.println("peer is:"+peer+"  history is:"+history);
		}
		else{
			history = connHistory.get(peer);
			System.out.println("history get peer from:"+connHistory.toString());
		}
...
//		通知社团检测对象：一个连接丢失了，这个对象也许在这一点上需要整个连接历史
	...
	public boolean shouldSendMessageToHost(Message m, DTNHost otherHost)
	{
		 ...
		 */
//		在此我们决定如何转发一个节点
//		DiBuBB的工作原理是它首先转发给网络中全局中心度最高的节点直到找到一个有消息目的地址的
//		节点（在同一本地社团中）。在此，它使用本地中心度跳数在社团内来转发消息
...
//		一旦消息转发到目的节点所在的社团，DiBuBB算法允许一个节点删除消息
